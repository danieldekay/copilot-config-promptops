...
**Delegate a fresh code review to the `dk.code-review` agent.**

Use a subagent to invoke the code review:

- The subagent must perform the full code-review workflow as defined in `dk.dk.dk.code-review.agent.md`
...
- **If assessment is "Needs Major Rework" or "Needs Changes" (with Critical/Major issues)**:
  ...
  - Ask: "Code review found issues that should be fixed. Options:
    1. **Auto-fix** — Delegate to dk.dk.dk.code-fix agent to automatically fix issues
...
**If user chose auto-fix in Step 4, delegate to the `dk.code-fix` agent:**
...
This dk.dk.dk.commit agent orchestrates a three-agent workflow:
...
This allows the dk.dk.dk.commit agent to:
