...
**Commit**: (will be filled by dk.dk.dk.commit agent)
...
- **If no code-review artifacts exist**: Stop and recommend running dk.dk.dk.code-review agent first
...
This agent is designed to be called by the dk.dk.dk.commit agent. The typical flow is:

1. **Commit agent** runs code review
2. **Code review** detects issues
3. **Commit agent** delegates to **dk.dk.dk.code-fix agent**
4. **Code-fix agent** applies fixes and updates artifacts
5. **Code-fix agent** hands back to **dk.dk.dk.commit agent**
6. **Commit agent** verifies fixes and creates commits in logical groups

The dk.dk.dk.commit agent will check for `FIX_UPDATE.md` to determine if fixes have been applied.
