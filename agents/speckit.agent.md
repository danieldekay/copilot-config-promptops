...
│
│  ── QUALITY STAGES ──
├── speckit.quality-gate   → Stage 6: Automated checks (lint, format, type, test, coverage)
├── speckit.autofix        → Stage 6b: Auto-fix gate failures (bounded loop)
├── speckit.review         → Stage 7: Structured code review with artifacts
│
│  ── INTEGRATION STAGES ──
├── speckit.commit         → Stage 8: Conventional commits + final verification
│
│  ── LEARNING STAGES ──
└── speckit.retro          → Stage 9: Compound learning & system improvement
```
...
4. **Announce** to the user which stage you're routing to and why
5. **Ask for confirmation** before proceeding (the user may want to override)
...
### Step 6: QUALITY GATE → `speckit.quality-gate`
...
### Step 6b: AUTO-FIX → `speckit.autofix`
...
|---------|-------------|-----------------|
| Quality gate (format/lint) | 2 | Auto-fix via `speckit.autofix` |
